...
function bones_related_posts() {
	...
	if($tags) {
        ...
        if($related_posts) {
            ...
            <?php echo '<li class="no_related_post">' . __( '没有相关的其他文章', 'bonestheme' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_query();
	echo '</ul>';
...
function bones_page_navi($before = '', $after = '') {
	...
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = __( "最新", 'bonestheme' );
	...
	echo '</li>';
	if ($end_page < $max_page) {
		$last_page_text = __( "最后", 'bonestheme' );
...
return '...  <a class="excerpt-read-more" href="'. get_permalink($post->ID) . '" title="'. __('', 'bonestheme') . get_the_title($post->ID).'">'. __('阅读全文', 'bonestheme') .'</a>';
}
